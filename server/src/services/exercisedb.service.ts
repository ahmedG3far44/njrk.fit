  ...
  try {
    ...
    if (exerciseCache[rawName]) {
      return {
        ...
        gifUrl: `${env.API_URL}/api/fitness/exercise-image/${exerciseCache[rawName]}`
      };
    }
    if (exerciseCache[cleanedName]) {
      return {
        exerciseId: exerciseCache[cleanedName],
        gifUrl: `${env.API_URL}/api/fitness/exercise-image/${exerciseCache[cleanedName]}`
      };
    }
    ...
    if (data && data.length > 0 && data[0].id) {
      return {
        ...
        gifUrl: `${env.API_URL}/api/fitness/exercise-image/${data[0].id}`,
      };
    }
