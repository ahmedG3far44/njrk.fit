...
router.post('/check-in', authMiddleware, async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      ...
      message: !result.didCheckIn
        ? 'Already checked in today.'
        : result.isFirstCheckIn
...
router.post('/apply-freeze', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const authReq = req as AuthRequest;
        const userId = authReq.user?.userId;
...
router.get('/insights', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const authReq = req as AuthRequest;
        const userId = authReq.user?.userId;
...
router.get('/rewards', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const authReq = req as AuthRequest;
        const userId = authReq.user?.userId;
...
router.post('/rewards/claim', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const authReq = req as AuthRequest;
        const userId = authReq.user?.userId;
