...
router.post('/invite', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const isAlreadyMember = inviter.familyMembers?.some(id => id.toString() === targetUserId);
    if (isAlreadyMember) {
      return res.status(400).json({ error: 'User is already in your family' });
    }
    ...
    if (existingInvitation) {
      ...
      if (existingInvitation.status === 'pending') {
        return res.status(400).json({ error: 'Invitation already sent and pending' });
      }
...
router.post('/remove', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthRequest;
    const userId = authReq.user?.userId;
    ...
    if (memberIndex === undefined || memberIndex === -1) {
      return res.status(400).json({ error: 'Member not found in your family list' });
    }
...
router.post('/cancel-invite', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthRequest;
    const userId = authReq.user?.userId;
    ...
    if (!invitation) {
      return res.status(404).json({ error: 'Pending invitation not found or not authorized' });
    }
...
router.get('/leaderboard', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const familyIds = [userId, ...(currentUser.familyMembers || [])].filter(Boolean) as (string | mongoose.Types.ObjectId)[];
...
router.get('/feed', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const recentActivities = await ProgressLog.find({ userId: { $in: familyIds } })
