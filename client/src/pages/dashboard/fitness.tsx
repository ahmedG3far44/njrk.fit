...
import { Target, Clock, Flame, Zap, CheckCircle, Loader2 } from 'lucide-react'
...
const FitnessPage = () => {
  ...
  const handleShare = async () => {
    console.log('Sharing fitness plan...', { timeRange })
    const url = `${window.location.origin}/dashboard/fitness?timeRange=${timeRange}`
    await shareCurrentView(
      'Njerka Fitness Plan',
      `Shared ${timeRange} fitness plan`,
      url
    )
  }

  const handlePrint = async () => {
    console.log('Printing fitness plan...', { timeRange })
    try {
      const blob = await fitnessService.exportPdf(timeRange)
      console.log('PDF Blob received:', blob)
      downloadBlob(blob, `fitness-plan-${timeRange}.pdf`)
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!workoutPlan || workoutPlan.sessions.length === 0 ? (
          ...
          {timeRange === 'day' ? (
            todaySession ? (
              ...
              <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-12 text-center">
                ...
                <p className="text-gray-500 font-medium">No workout scheduled for today</p>
                <p className="text-gray-400 text-sm mt-1">Enjoy your rest day!</p>
              </div>
            )
          ) : (
