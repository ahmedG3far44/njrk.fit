...
export const RecipeDetail: React.FC<RecipeDetailProps> = ({
  ...
  const [currentRecipe, setCurrentRecipe] =
    useState<RecipeDetailProps["recipe"]>(recipe);
  ...
  const getIngredients = () => {
    const ingredients = currentRecipe?.ingredients?.length
      ? currentRecipe.ingredients
      : defaultRecipe.ingredients;
    return ingredients
      .map((ing: string | { name: string; _id?: string }) =>
  ...
  const getSteps = () => {
    const steps = currentRecipe?.steps?.length
      ...
      : currentRecipe?.instructions?.length
        ...
        : defaultRecipe.steps;
    return steps
      .map((step: string | { name: string; _id?: string }) =>
  ...
  const data = {
    ...
    image:
      currentRecipe?.image ||
      getDynamicMealImage(
        currentRecipe?.name || recipe?.name || defaultRecipe.name,
      ),
  };
  ...
  const handleRefine = async () => {
    ...
    try {
      ...
      const response = await nutritionService.refine(mealId, {
        ...
        <div className="flex flex-col md:flex-row min-h-full">
          ...
          <div className="md:w-1/2 relative h-[250px] sm:h-[400px] md:min-h-screen">
            <Button
              ...
              size="icon"
              onClick={onClose}
              className="absolute top-4 end-4 z-30 bg-white/90 shadow-sm"
            >
              <X className="w-6 h-6" />
            </Button>
              ...
              {isRegenerating ? (
                ...
                >
                  <img
                    ...
                    className="absolute inset-0 w-full h-full object-cover blur-sm transition-all duration-500"
                  />
            ...
            <div className="p-5 sm:p-8">
              ...
              <div className="flex gap-3 mb-6">
                {[
                  ...
                  <div
                    key={nut.label}
                    className={`flex-1 p-3 rounded-xl text-center ${nut.color}`}
                  >
                ...
                {canRefine && (
                  <div className="border-t border-slate-100 pt-6">
                    <h3 className="font-bold text-slate-900 mb-3 flex items-center gap-2">
                      ...
                      {t("recipeDetail.refineWithAI")}
                    </h3>
                    <p className="text-xs text-slate-400 mb-3">
                      {t("recipeDetail.refineSubtext")}
                    </p>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder={
                          replacementsLeft <= 0
                            ? t("recipeDetail.refinePlaceholderLocked")
                            : t("recipeDetail.refinePlaceholder")
                        }
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-xs text-slate-400">
                        {t("recipeDetail.replacementsLeft")}{" "}
                        <strong>{replacementsLeft}</strong>/3
                      </span>
                    </div>
                    ...
                    >
                      ...
                      />
                      {replacementsLeft <= 0
                        ? t("recipeDetail.noTriesLeft")
                        : t("recipeDetail.regenerateMeal")}
                    </Button>
                  </div>
                )}
              </div>
