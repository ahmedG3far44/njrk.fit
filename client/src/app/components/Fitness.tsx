        ...
        {selectedWorkout && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full h-full sm:max-w-2xl sm:h-auto sm:rounded-3xl overflow-hidden shadow-2xl sm:max-h-[88vh] flex flex-col"
            >
              ...
              <div className="p-4 sm:p-6 overflow-y-auto flex-1">
                <div className="grid grid-cols-3 gap-1.5 sm:gap-3 mb-4 sm:mb-6">
                  {[
                    ...
                    <div
                      key={s.label}
                      className="bg-slate-50 rounded-xl sm:rounded-2xl p-2 sm:p-3 text-center border border-slate-100"
                    >
                      <s.icon className="w-3.5 h-3.5 text-green-700 mx-auto mb-1" />
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 sm:gap-3 min-w-0 mr-2">
                            ...
                            <div
                              className={`font-bold text-xs sm:text-sm truncate ${isCompleted ? "text-green-700 line-through" : "text-slate-800"}`}
                            >
                              {ex.name}
                            </div>
                            {ex.gifUrl && (
                          ...
                          <div className="flex gap-1.5 sm:gap-3 text-[9px] sm:text-xs font-semibold text-slate-500 uppercase tracking-wider flex-shrink-0">
                            <div className="flex flex-col items-center">
                              <span
                                className={`text-xs sm:text-base font-black ${isCompleted ? "text-green-600" : "text-green-700"}`}
                              >
                                {ex.sets}
                              </span>
        ...
        {showGifModal && currentGifUrl && (
          ...
          >
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 16 }}
              className="bg-white w-full h-screen sm:max-w-md sm:h-auto sm:rounded-3xl overflow-hidden shadow-2xl"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="md:col-span-2 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                        ...
                        <div className="space-y-2">
                          {filteredUpcoming.slice(0, 5).map((session, i) => (
                            ...
                            >
                              <div
                                className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-sm flex-shrink-0 ${session.done ? "bg-green-100 text-green-600" : "bg-slate-100 text-slate-600 group-hover:bg-green-100 group-hover:text-green-700 transition-colors duration-150"}`}
                              >
                                {session.done ? (
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span
                                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${typeConfig[session.type]?.bg || "bg-slate-100"} ${typeConfig[session.type]?.text || "text-slate-500"}`}
                                  >
                                    {session.type}
                                  </span>
                      ...
                      <div className="bg-gradient-to-br from-green-800 to-green-700 p-6 rounded-3xl text-white relative overflow-hidden">
                        ...
                        <p className="text-green-200 text-xs mb-5 relative z-10">
                          ...
                          {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => {
                            ...
                            return (
                              <div key={i} className="flex items-center gap-2">
                                <div
                                  className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${isDone ? "bg-white" : hasSession ? "bg-white/30" : "bg-transparent"}`}
                                >
                                  {isDone && (
                                ...
                                <span
                                  className={`text-xs font-medium ${isDone ? "text-white" : hasSession ? "text-white/70" : "text-white/30"}`}
                                >
                                  {d}
                                </span>
                              </div>
                    ...
                    <div className="divide-y divide-slate-50">
                      {weeklySessions.map((session, i) => (
                        <motion.div
                          ...
                          className={`flex items-center gap-3 sm:gap-4 p-3.5 sm:p-5 hover:bg-slate-50 transition-colors duration-150 cursor-pointer group ${session.done ? "opacity-70" : ""}`}
                        >
                          <div
                            className={`w-9 h-9 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center font-bold text-xs sm:text-sm flex-shrink-0 transition-colors ${session.done ? "bg-green-100 text-green-600" : "bg-slate-100 text-slate-600 group-hover:bg-green-100 group-hover:text-green-700"}`}
                          >
                            {session.done ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <h4
                              className={`font-bold text-xs sm:text-sm truncate ${session.done ? "text-slate-400 line-through" : "text-slate-900 group-hover:text-green-700"} transition-colors`}
                            >
                              {session.title}
                            </h4>
                            <div className="flex items-center gap-1.5 sm:gap-2 mt-1">
                              <span
                                className={`text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded-full ${typeConfig[session.type]?.bg || "bg-slate-100"} ${typeConfig[session.type]?.text || "text-slate-500"}`}
                              >
                                {session.type}
                              </span>
