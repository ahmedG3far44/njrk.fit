    ...
    switch (step) {
          ...
          <div className="space-y-4 sm:space-y-5">
            <div className="space-y-1.5 sm:space-y-2">
              <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Username
              </label>
              <input
                type="text"
                value={formData.name}
                onChange={e => { update({ name: e.target.value }); clearError('name'); }}
                className={`w-full px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl border outline-none transition-all text-sm ${errors.name ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-border focus:ring-2 focus:ring-green-600'}`}
            ...
            <div className="flex flex-col gap-3.5 sm:gap-4">
              <div className="space-y-1.5 sm:space-y-2">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                  <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Age
                </label>
                <input
                  type="number"
                  value={formData.age}
                  onChange={e => { update({ age: e.target.value }); clearError('age'); }}
                  className={`w-full px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl border outline-none transition-all text-sm ${errors.age ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-border focus:ring-2 focus:ring-green-600'}`}
                  placeholder="25"
                  min={10} max={100}
                />
              ...
              <div className="space-y-1.5 sm:space-y-2">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground">Gender</label>
                  ...
                  {['Male', 'Female'].map(g => (
                    <button
                      ...
                      className={`flex-1 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border-2 text-xs sm:text-sm font-semibold transition-all ${formData.gender === g
                        ? 'border-forest-canopy bg-forest-canopy/15 text-forest-canopy'
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <div className="space-y-1.5 sm:space-y-2">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                  <Ruler className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Height (cm)
                </label>
                <input
                  type="number"
                  value={formData.height}
                  onChange={e => { update({ height: e.target.value }); clearError('height'); }}
                  className={`w-full px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl border outline-none transition-all text-sm ${errors.height ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-border focus:ring-2 focus:ring-green-600'}`}
                  placeholder="175"
                />
              ...
              <div className="space-y-1.5 sm:space-y-2">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                  <Scale className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Weight (kg)
                </label>
                <input
                  type="number"
                  value={formData.weight || ''}
                  onChange={e => { update({ weight: Number(e.target.value) }); clearError('weight'); }}
                  className={`w-full px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl border outline-none transition-all text-sm ${errors.weight ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-border focus:ring-2 focus:ring-green-600'}`}
                  placeholder="70"
                />
        ...
        return (
          <div className="space-y-5 sm:space-y-6">
            <div className="space-y-2 sm:space-y-3">
              <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Religion / Fasting Periods
              </label>
              <p className="text-[11px] sm:text-xs text-muted-foreground">Helps us respect your fasting calendars (Ramadan, Lent, etc.)</p>
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                  <ChefHat className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Food Preferences
                </label>
                <span className="text-[10px] sm:text-xs text-muted-foreground font-medium">
                  {formData.foodPreferences.length} selected
                </span>
              </div>
              <p className="text-[11px] sm:text-xs text-muted-foreground">Select the foods you want to include in your diet</p>

              {FOOD_PREFERENCES.map((category) => (
                <div key={category.category} className="space-y-1.5 sm:space-y-2">
                  <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground uppercase tracking-wider">{category.category}</p>
      ...
      case 2:
        return (
          <div className="space-y-5 sm:space-y-6">
            <div className="space-y-2 sm:space-y-3">
              <label className="text-xs sm:text-sm font-semibold text-card-foreground">Allergies & Intolerances</label>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl border border-border focus:ring-2 focus:ring-green-600 outline-none transition-all text-xs sm:text-sm"
            ...
            <div className="space-y-2 sm:space-y-3">
              <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                <Activity className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Activity Level
              </label>
              <div className="space-y-1.5 sm:space-y-2">
                {[
                  ...
                  <button
                    ...
                    className={`w-full p-2.5 sm:p-3.5 rounded-lg sm:rounded-xl border-[1.5px] sm:border-2 text-start flex items-center gap-2.5 sm:gap-4 transition-all ${formData.activityLevel === level.label
                      ? 'border-forest-canopy bg-forest-canopy/15'
                      : 'border-border hover:border-foreground/20 bg-card'
                      }`}
                  >
                    <span className="text-xl sm:text-2xl flex-shrink-0">{level.emoji}</span>
                    <div className="min-w-0 flex-1">
                      <div className={`font-semibold text-xs sm:text-sm ${formData.activityLevel === level.label ? 'text-forest-canopy' : 'text-card-foreground'}`}>{level.label}</div>
                      <div className="text-[10px] sm:text-xs text-muted-foreground truncate">{level.desc}</div>
                    </div>
                    {formData.activityLevel === level.label && (
                      <div className="ms-auto w-4 h-4 sm:w-5 sm:h-5 bg-forest-canopy rounded-full flex items-center justify-center flex-shrink-0">
        ...
        return (
          <div className="space-y-4 sm:space-y-6">
            <div className='grid grid-cols-2 gap-2.5 sm:gap-3'>
              <div className="space-y-1.5 sm:space-y-2">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground">Select your goal</label>
                <select
                  onChange={(e) => { update({ goal: e.target.value }); clearError('goal'); }}
                  value={formData.goal}
                  className={`w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border outline-none transition-all text-xs sm:text-sm text-card-foreground ${errors.goal ? 'border-red-500' : 'border-border focus:ring-2 focus:ring-green-700'}`}
                >
              ...
              <div className="space-y-1.5 sm:space-y-2">
                <label className="text-xs sm:text-sm font-semibold text-card-foreground">Target Weight (kg)</label>
                <input
                  onChange={(e) => { update({ targetWeight: Number(e.target.value) }); clearError('targetWeight'); }}
                  type="number"
                  value={formData.targetWeight || ''}
                  className={`w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border outline-none transition-all text-xs sm:text-sm text-card-foreground ${errors.targetWeight ? 'border-red-500' : 'border-border focus:ring-2 focus:ring-green-700'}`}
            ...
            <div className="space-y-1.5 sm:space-y-2 mt-4 sm:mt-5">
              <label htmlFor="dreamGoal" className="text-xs sm:text-sm font-semibold text-card-foreground">Describe Your Dream Body & Health Goal</label>
              <textarea
                value={formData.dreamGoal}
                onChange={(e) => { update({ dreamGoal: e.target.value }); clearError('dreamGoal'); }}
                className={`w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border outline-none transition-all text-xs sm:text-sm text-card-foreground leading-relaxed resize-none ${errors.dreamGoal ? 'border-red-500' : 'border-border focus:ring-2 focus:ring-green-700'}`}
                rows={4}
              ...
              <div className="flex justify-between text-[10px] sm:text-xs text-muted-foreground">
                ...
                <span className={formData.dreamGoal.length >= 10 ? 'text-forest-canopy' : ''}>
            ...
            <div className="space-y-1.5 sm:space-y-2">
              <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> Target Date
              </label>
              <p className="text-[11px] sm:text-xs text-muted-foreground">When do you want to achieve this goal?</p>
              <input
                type="date"
                value={formData.goalDate}
                onChange={(e) => { update({ goalDate: e.target.value }); clearError('goalDate'); }}
                min={new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}
                className={`w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border outline-none transition-all text-xs sm:text-sm text-card-foreground ${errors.goalDate ? 'border-red-500' : 'border-border focus:ring-2 focus:ring-green-700'}`}
              />
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              {[
                ...
                <button
                  ...
                  className="p-2.5 sm:p-3 bg-muted border border-border rounded-lg sm:rounded-xl text-[10px] sm:text-xs text-muted-foreground hover:bg-forest-canopy/10 hover:border-forest-canopy/30 hover:text-forest-canopy transition-all text-start font-medium line-clamp-1"
                >
                  + {suggestion}
                </button>
              ))}
      ...
      case 4:
        return (
          <div className="space-y-6 sm:space-y-8">
            <div className="space-y-3 sm:space-y-4">
              <label className="text-xs sm:text-sm font-semibold text-card-foreground flex items-center gap-2">
                <Languages className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-forest-canopy" /> {t('onboarding.language')}
              </label>
              <p className="text-[11px] sm:text-xs text-muted-foreground">{t('onboarding.languageSubtext')}</p>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-4">
                <button
                  ...
                  className={`flex flex-col items-center gap-3 p-6 sm:p-8 rounded-xl sm:rounded-2xl border-2 transition-all ${formData.language === 'en'
                    ? 'border-forest-canopy bg-forest-canopy/15 text-forest-canopy shadow-md shadow-forest-canopy/10'
                ...
                >
                  ...
                  <span className="text-[10px] sm:text-xs text-muted-foreground text-center">{t('onboarding.englishDesc')}</span>
                  {formData.language === 'en' && (
                    <div className="w-5 h-5 sm:w-6 sm:h-6 bg-forest-canopy rounded-full flex items-center justify-center mt-1">
                ...
                >
                  ...
                  <span className="text-[10px] sm:text-xs text-muted-foreground text-center">{t('onboarding.arabicDesc')}</span>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-green-50/30 to-emerald-50/30 p-3 sm:p-4">
      <motion.div
        ...
        className="w-full max-w-lg bg-card rounded-2xl sm:rounded-3xl shadow-xl border border-border overflow-hidden"
      >
        {/* Progress Bar */}
        ...
        {/* Step Indicators */}
        <div className="flex justify-between items-center px-4 sm:px-8 py-3.5 sm:py-5 border-b border-border">
          {STEP_ICONS.map((Icon, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center transition-all ${i < step ? 'bg-forest-canopy text-white' :
                i === step ? 'bg-gradient-to-br from-forest-deep to-forest-canopy text-white shadow-lg shadow-forest-canopy/20' :
                  'bg-muted text-muted-foreground'
                }`}>
                {i < step ? <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
              </div>
              <span className={`text-[10px] font-semibold hidden sm:block ${i === step ? 'text-forest-canopy' : 'text-muted-foreground'}`}>
                {STEP_LABELS[i]}
              </span>
            </div>
          ))}
        ...
        <div className="p-5 sm:p-8 md:p-10">
          ...
          <div className="mb-5 sm:mb-7">
            <div className="text-xs sm:text-sm font-bold text-forest-canopy uppercase tracking-wider mb-1.5 sm:mb-2">
              {t('onboarding.stepLabel')} {step + 1} {t('onboarding.of')} 5
            </div>
            <h1 className="text-xl sm:text-2xl font-bold text-card-foreground leading-tight">
              {[
          ...
          {/* Navigation */}
          <div className="flex justify-between items-center mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-border">
            <button
              type="button"
              onClick={() => setStep(s => Math.max(0, s - 1))}
              disabled={step === 0 || isProcessing}
              className="flex items-center gap-1.5 text-muted-foreground font-semibold hover:text-card-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-all text-xs sm:text-sm"
            >
