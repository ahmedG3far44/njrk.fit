...
export const groceryService = {
  ...
  async toggleItem(itemName: string, checked: boolean): Promise<{ success: boolean }> {
    ...
    //   return { success: true }
    // }
    const data = await api.post<{ success: boolean }>('/groceries/toggle-item', {
      itemName,
      checked,
    })
    return data
  ...
  async addItem(name: string, quantity = '1', category = 'Other'): Promise<{ success: boolean }> {
    ...
    //   return { success: true }
    // }
    const data = await api.post<{ success: boolean }>('/groceries/add-item', {
      name,
      quantity,
      category,
    })
  ...
  async createShare(): Promise<{ success: boolean; shareUrl: string }> {
    ...
    // }
    const data = await api.post<{ success: boolean; shareUrl: string }>('/groceries/share')
    return data
  },
  ...
  async getSharedList(token: string): Promise<SharedListData> {
    ...
    //   }
    // }
    const data = await api.get<SharedListData>(`/groceries/shared/${token}`)
    return data
  },
}