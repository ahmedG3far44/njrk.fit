...
export const communityService = {
  async getFeed(page = 1, limit = 10): Promise<{ posts: Post[]; pagination: Pagination }> {
    ...
    const { data } = await api.get<{ posts: Post[]; pagination: Pagination }>('/community/feed', {
      params: { page, limit },
    })
    return data
  },
  ...
  async createPost(content: string, media?: File): Promise<{ post: Post }> {
    if (USE_MOCK) {
      const newPost = {
        _id: 'post-' + Date.now(),
        content,
        userId: { _id: 'current-user', name: 'You', avatarUrl: '' },
        mediaUrl: '',
        likeCount: 0,
        commentCount: 0,
        isLiked: false,
    ...
    const { data } = await api.post<{ post: Post }>('/community/posts', formData, {
  ...
  async toggleLike(postId: string): Promise<{ success: boolean; liked: boolean; likeCount: number }> {
    if (USE_MOCK) {
      ...
      if (post) {
        ...
        return { success: true, liked: post.isLiked, likeCount: post.likeCount }
      }
    }
