...
export const progressService = {
  async getDashboard(timeframe = '7days'): Promise<ProgressDashboard> {
    ...
    const { data } = await api.get<ProgressDashboard>('/progress/dashboard', {
      params: { timeframe },
    })
    return data
  },
  ...
  async getHistory(page = 1, limit = 10): Promise<{ logs: ProgressLog[]; pagination: ProgressPagination }> {
    ...
    const { data } = await api.get<{ logs: ProgressLog[]; pagination: ProgressPagination }>('/progress/history', {
      params: { page, limit },
    })
    return data
  },
  ...
  async extractInBody(file: File): Promise<{ extracted: ExtractedInBody }> {
    ...
    const { data } = await api.post<{ extracted: ExtractedInBody }>('/progress/extract-inbody', formData, {
  ...
  async getFeelings(days = 7): Promise<{ feelings: FeelingsLog[]}> {
    ...
    const { data } = await api.get<{ feelings: FeelingsLog[] }>('/progress/feelings', {
      params: { days },
    })
    return data
  },
  ...
  async logProgress(data: {
    ...
    const { data: response } = await api.post<{ progressLog: ProgressLog }>('/progress/log', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    return response
  },
