...
export const gamificationService = {
  ...
  async checkIn(): Promise<{
    ...
    const { data } = await api.post('/gamification/check-in', { timezoneOffset })
    return data
  },
  ...
  async freeze(): Promise<{ success: boolean; availableFreezes: number; message: string }> {
    ...
    const timezoneOffset = getTimezoneOffset()
    const { data } = await api.post('/gamification/freeze', { timezoneOffset })
    return data
  },
  ...
  async claimReward(rewardId: string): Promise<{
    ...
    const { data } = await api.post('/gamification/rewards/claim', { rewardId })
    return data
  },
}